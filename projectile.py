...
class Kunai:
    ...
    def __init__(self, game, position = (0, 0), velocity = (0, 0)):
        ...
        if not self.surf:
            Kunai.surf = pygame.image.load("images/kunai_2.png")
            #Kunai.surf.set_colorkey((255, 255, 255)) #This doesn't work idk why, posted a question on Stack
    ...
    def launch(self, velocity):
        self.velocity = velocity.copy() #Initiate

    def update(self, dt, events):
        self.last_position = self.position.copy()
        self.position += self.velocity * dt #DT is a bit unstable, but timer is worse
        if self.velocity.magnitude() > 1:
            self.direction = self.velocity * (1/self.velocity.magnitude()) #Don't touch, inverse is necessary
        ...
        if not self.pickup and not self.stuck:
            ...
            if self.gravity:
                ...
                self.velocity += Pose((0, 5000)) * dt #Slight downward force

            if dist != 0:
                increment *= 1/increment.magnitude()
                increment *= 10
                traveled = 0
                current = self.last_position.copy() #Draws a tail behind the kunai
                while traveled < dist:
                    self.shadows.append([current, 60]) #Group of tails
                    current += increment
        ...
        for item in self.shadows[:]:
            item[1] -= 1000 * dt
            if item[1] < 0:
                self.shadows.remove(item) #Remove surplus tails
