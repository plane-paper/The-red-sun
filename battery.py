...
class Battery:
    ...
    def update(self, dt, events):
        ...
        if diff.magnitude() < 50:
            ...
            self.game.player.charge += self.capacity #This fails idk why, two enemies give the same charge
    ...
    def draw(self, screen, offset=(0, 0)):
        ...
        x = offset[0] - self.surf.get_width()//2 + self.position.x #Make base surface
        y = offset[1] - self.surf.get_width()//2 + self.position.y
        screen.blit(self.surf, (x, y), special_flags=pygame.BLEND_ADD)

        x = offset[0] - self.glow.get_width()//2 + self.position.x #Make glow
